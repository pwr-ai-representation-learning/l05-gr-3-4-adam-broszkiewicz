...
def visualize_embeddings(x_all: Tensor, y_all: Tensor, encoder: nn.Module, window_size: int):
    x_all = x_all.numpy()
    y_all = y_all.numpy()
    ...
    with torch.no_grad():
        ...
        input_windows = torch.tensor(windows).to(device)
        encodings = encoder(input_windows).cpu().numpy()

    tnc_embedding = TSNE(n_components=2).fit_transform(encodings)
